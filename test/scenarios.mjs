...
const seqPatient = await post('/api/patients', {
  scenarios: ['claims/02-cpt-90837-denied', 'claims/99-default-paid-with-patient-resp'] });
...
const ordered = first.matchedStub === 'claims/02-cpt-90837-denied'
  && second.matchedStub === 'claims/99-default-paid-with-patient-resp'
  && third.matchedStub === 'claims/99-default-paid-with-patient-resp';
...
const a = await post('/api/patients', { scenarios: ['eligibility/99-default-active'] });
const b = await post('/api/patients', { scenarios: ['eligibility/99-default-active'] });
